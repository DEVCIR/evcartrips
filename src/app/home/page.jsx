...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <motion.div 
        ref={heroRef}
        className="max-md:bg-black sm:px-4 md:px-6 lg:px-8 pt-4 pb-12 sm:pb-16 min-h-[65vh] md:min-h-[50vh] flex flex-col justify-start rounded-b-[24px] sm:rounded-b-[32px] md:rounded-b-[40px]"
      ...
      <motion.div 
        ref={formRef}
        className="px-8 -mt-48 md:-mt-22 pb-8 xl:flex xl:justify-between xl:-mt-36 xl:mr-16 xl:mb-0"
        initial={{ y: 50, opacity: 0 }}
        animate={isFormInView ? { y: 0, opacity: 1 } : { y: 50, opacity: 0 }}
        transition={{ duration: 0.8 }}
      >
        <motion.div 
          className="z-10 ml-48 mt-28 max-xl:hidden"
          initial={{ x: -100, opacity: 0 }}
          animate={isFormInView ? { x: 0, opacity: 1 } : { x: -100, opacity: 0 }}
          transition={{ duration: 0.8, delay: 0.2 }}
        >
          <motion.h3 
            className="text-[24px] font-medium text-[#FFFFFFCF]"
          ...
          <motion.h1 
            className="text-[48px] font-bold text-white leading-14"
            initial={{ y: 30, opacity: 0 }}
        ...
        <motion.div 
          className=" z-50 max-w-full md:max-w-10/12 xl:w-[543px] xl:h-[685px] max-xl:mx-auto"
          initial={{ x: 100, opacity: 0 }}
          animate={isFormInView ? { x: 0, opacity: 1 } : { x: 100, opacity: 0 }}
